...
RESULT_FILE_REGEX = re.compile(r"""
 (\w+)      # Benchmark 
 \.         # Delimiter
 (\w+)      # Architecture 
 \.         # Delimiter
...
def dataframe_to_boxplot(df, arch, hue='Experiment'):
    ...
    ax = df.plot.bar(x='Benchmark', y='% Overhead',
    ...
    plt.ylabel('% Overhead')
    plt.title('{} overhead from \"{}\"- NPB class B'.format(arch, df['Experiment'][0]))
    plt.show()
...
if __name__ == '__main__':
    for short_hash in ['d97be7cf96dc']:
        df_overhead = get_overhead_df('npb/results/c6780392', 'npb/results/' + short_hash, arch='aarch64')
        dataframe_to_boxplot(df_overhead, arch='aarch64')
        df_overhead = get_overhead_df('npb/results/c6780392', 'npb/results/' + short_hash, arch='x86_64')
