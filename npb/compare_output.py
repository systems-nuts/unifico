...
if __name__ == '__main__':
    ...
    arg_parser = argparse.ArgumentParser(
        description=
        'Compare NPB results for two different LLVM versions, with one '
        'being the baseline.')

    arg_parser.add_argument('baseline_dir',
                            type=str,
                            help='path for baseline result directory')

    arg_parser.add_argument('test_dir',
                            type=str,
                            help='path for test result directory')
    ...
    for arch in args.archs.split(','):
        df_overhead = get_overhead_df(args.baseline_dir, args.test_dir, arch,
                                      args.npb_class)
