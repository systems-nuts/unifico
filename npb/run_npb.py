...
if __name__ == '__main__':
    ...
    parser.add_argument('--suite-list', action="store", required=True,
    ...
    if args.full_core_run:  # Test scalability with only one thread per physical core
        thread_list = [1] + [x for x in range(1, topology.core_num + 1) if x % 2 == 0]
    elif args.full_thread_run:  # Test scalability with one thread per logical processing unit (PU)
        thread_list = [1] + [x for x in range(1, topology.cpu_num + 1) if x % 2 == 0]
