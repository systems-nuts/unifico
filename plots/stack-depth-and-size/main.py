...
def trim_non_push_and_sub_instructions(instructions):
    i = 0
    while i != len(instructions) and (('push   %' in instructions[i]) or ('sub    $0x' in instructions[i])):
        i += 1
...
def stack_states_with_size_and_duration(functions_sizes, perf_txt=None, benchmark_name=None):
...
def parse_and_calculate_function_sizes(asm_file):
    ...
    functions_sizes = {}
    for f in functions:
        functions_sizes[f[0]] = calculate_function_size(trim_non_push_and_sub_instructions(f[1]))
...
def main():
    for benchmarks_name in BENCHMARKS_NAMES:
        ...
        with zipfile.ZipFile('./' + benchmarks_name + '.stack.zip', 'r') as zip_ref:
            zip_ref.extractall('./')
        spec_sizes = {}
        for benchmark_folder in [name for name in os.listdir(benchmarks_folder) if
                                 os.path.isdir(os.path.join(benchmarks_folder, name))]:
            asm_file = os.path.join(benchmarks_folder, benchmark_folder, 'asm.s')
            functions_sizes = parse_and_calculate_function_sizes(asm_file)

            benchmark_name = BENCHMARKS_FOLDERS[benchmarks_name][benchmark_folder]
            perf_txt = os.path.join(benchmarks_folder, benchmark_folder, 'out.folded')
            stack_states = stack_states_with_size_and_duration(functions_sizes, perf_txt, benchmark_name)

            stack_states_sizes = [(sum(stack_state[0]), stack_state[1]) for stack_state in stack_states]
            sizes = []
