...
void jacld(int k)
{
    ...
    for (j = jst; j < jend; j++) {
        for (i = ist; i < iend; i++) {
            ...
            d[j][i][0][1] = -dt * 2.0 * (tx1 * r43 + ty1 + tz1) * c34 * tmp2 *
                            u[k][j][i][1];
            d[j][i][1][1] = 1.0 +
            ...
            d[j][i][0][2] = -dt * 2.0 * (tx1 + ty1 * r43 + tz1) * c34 * tmp2 *
                            u[k][j][i][2];
            d[j][i][1][2] = 0.0;
            d[j][i][2][2] = 1.0 +
            ...
            d[j][i][0][3] = -dt * 2.0 * (tx1 + ty1 + tz1 * r43) * c34 * tmp2 *
                            u[k][j][i][3];
            d[j][i][1][3] = 0.0;
            d[j][i][2][3] = 0.0;
            d[j][i][3][3] = 1.0 +
            ...
            a[j][i][3][3] =
                -dt * tz2 * (2.0 - C2) * (u[k - 1][j][i][3] * tmp1) -
            ...
            b[j][i][2][2] =
                -dt * ty2 * ((2.0 - C2) * (u[k][j - 1][i][2] * tmp1)) -
            ...
            c[j][i][1][1] =
                -dt * tx2 * ((2.0 - C2) * (u[k][j][i - 1][1] * tmp1)) -
