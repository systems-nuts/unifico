...
void blts(int ldmx, int ldmy, int ldmz, int nx, int ny, int nz, int k,
          ...
          double d[ldmy][ldmx / 2 * 2 + 1][5][5], int ist, int iend, int jst,
          int jend, int nx0, int ny0)
{
    ...
    for (j = jst; j < jend; j++) {
        for (i = ist; i < iend; i++) {
            for (m = 0; m < 5; m++) {
                vk[j][i][m] =
                    vk[j][i][m] - omega * (ldz[j][i][0][m] * vkm1[j][i][0] +
    ...
    for (j = jst; j < jend; j++) {
        for (i = ist; i < iend; i++) {
            for (m = 0; m < 5; m++) {
                tv[m] =
                    vk[j][i][m] - omega * (ldy[j][i][0][m] * vk[j - 1][i][0] +
            ...
            tv[1] = tv[1] - tmat[1][2] * vk[j][i][2] -
                    tmat[1][3] * vk[j][i][3] - tmat[1][4] * vk[j][i][4];
            vk[j][i][1] = tv[1] / tmat[1][1];

            tv[0] = tv[0] - tmat[0][1] * vk[j][i][1] -
                    tmat[0][2] * vk[j][i][2] - tmat[0][3] * vk[j][i][3] -
                    tmat[0][4] * vk[j][i][4];
