...
#pragma omp critical
    ...
    for (uint32_t nodeIdx = 0; nodeIdx < csrGraph.numNodes; ++nodeIdx) {
        if (nodeLevel[nodeIdx] != nodeLevelRef[nodeIdx]) {
            PRINT_ERROR("Mismatch at node %u (CPU sequential result = level "
                        "%u, CPU parallel result = level %u)",
                        nodeIdx, nodeLevelRef[nodeIdx], nodeLevel[nodeIdx]);
        }
    }
