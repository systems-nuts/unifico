...
int main(int argc, char *argv[])
{
    ...
    for (step = 0; step <= niter; step++) {
        if (step == 1) {
            // reset the solution and start the timer, keep track of total no
            // elms
            r_init((double *)ta1, ntot, 0.0);
            time = 0.0;
            nelt_tot = 0.0;
    ...
    print_results("UA", Class, REFINE_MAX, 0, 0, niter, tmax, mflops,
                  "    coll. point advanced", verified, NPBVERSION, COMPILETIME,
