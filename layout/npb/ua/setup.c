...
void setdef()
{
    ...
    for (i = 0; i < LX1; i++) {
        for (j = 0; j < LX1; j++) {
            for (ip = 0; ip < LX1; ip++) {
                wdtdr[j][i] =
                    wdtdr[j][i] + wxm1[ip] * dxm1[i][ip] * dxm1[j][ip];
            }
        }
    }
}
...
void top_constants()
{
    // f_e_ef[f][e] returns the other face sharing the e'th local edge of face
    // f.
    f_e_ef[0][0] = 5;
    f_e_ef[0][1] = 2;
    f_e_ef[0][2] = 4;
    ...
    // le_arr[n][j][i] returns the i'th elements among the four children
    // elements
