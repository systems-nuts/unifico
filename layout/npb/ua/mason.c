...
static void mor_ne(int mor_v[3], int nn, int edge, int face, int edge2,
                   int face2, int ntemp, int iel);
...
void mortar()
{
    ...
    for (iel = 0; iel < nelt; iel++) {
        ...
        if (cb6 == 0) {
            if (cb4 == 0 || cb4 == 1) {
                // if face 6 is of type 0 and face 4 is of type 0 or type 1, the
                // edge shared by face 4 and 6 (edge 10) will generate new
                // mortar point indices.
                newe[iel] = newe[iel] + 1;
                eassign[iel][10] = true;
            }
            if (cb1 != 3) {
                // if face 1 is of type 3, the edge shared by face 6 and 1 (edge
                // 0) will generate new mortar points indices.
                newe[iel] = newe[iel] + 1;
                eassign[iel][0] = true;
            }
            if (cb3 != 3) {
            ...
            if (cb4 == 0) {
                ...
                // If face 6 and face 4 both are of type 1, ntemp is the
                // neighbor element on face 4.
                ntemp = sje[iel][3][0][0];

                // if ntemp's face 6 is not noncoforming or the neighbor element
                // of ntemp on face 6 has an element index larger than iel, the
                // edge shared by face 6 and 4 (edge 10) will generate new
                // mortar point indices.
                if (cbc[ntemp][5] != 3 || sje[ntemp][5][0][0] > iel) {
                    ...
                    if (cbc[ntemp][5] == 2) {
                        // The neighbor element of iel, neighbored by edge 10,
                        // is sje[ntemp][5][0][0] (the neighbor element of ntemp
                        // on ntemp's face 6).
                        diagn[iel][10][0] = sje[ntemp][5][0][0];
                        // The neighbor element of iel, neighbored by edge 10
                        // shares the ijel[iel][5][1] part of edge 10 of iel
                        diagn[iel][10][1] = ijel[iel][5][1];
                        // edge 9 of element sje[ntemp][5][0][0] (the neighbor
                        // element of ntemp on ntemp's face 6) is a
                        // nonconforming edge
                        ncon_edge[sje[ntemp][5][0][0]][9] = true;
                        // if_1_edge[iel][n]=true indicates that iel is of a
                        // smaller
                        // size than its neighbor element, neighbored by edge n
                        // of iel only.
    ...
    for (iel = 0; iel < nelt; iel++) {
        ...
        for (i = 0; i < 6; i++) {
            ...
            if (i < 2) {
                ...
                // i loops over faces. Only 4 faces need to be examed for edge
                // visit. On face 1, edge 0,1,2 and 3 will be visited. On face
                // 2, edge 4,5,6 and 7 will be visited. On face 3, edge 8 and 9
                // will be visited and on face 4, edge 10 and 11 will be
                // visited. The 12 edges can be covered by four faces, there is
                // no need to visit edges on face 5 and 6.  So ne is set to be
                // 0. However, i still needs to loop over 4 and 5, since the
                // interiors of face 5 and 6 still need to be visited.
            }
            else {
                ne = 0;
                space = 1;
            ...
            for (ie = 0; ie < ne; ie += space) {
                ...
                if (eassign[iel][edge_g]) {
                    ...
                    // asssign mor_v  to local edge ie2 of face face2 on element
                    // iel
                    mor_edge(ie2, face2, iel, mor_v);

                    // There are some neighbor elements also sharing this edge.
                    // Assign mor_v to neighbor element, neighbored by face i.
                    if (cbc[iel][i] == 2) {
                        ntemp = sje[iel][i][0][0];
                    ...
                    // assign mor_v  to neighbor element neighbored by face
                    // face2
                    if (cbc[iel][face2] == 2) {
                    ...
                    if (!if_1_edge[iel][edgenumber[i][ie]]) {
                        ...
                        if (diagn[iel][edgenumber[i][ie]][0] != -1) {
                            ntemp = diagn[iel][edgenumber[i][ie]][0];
                            mor_ne(mor_v, diagn[iel][edgenumber[i][ie]][1], ie,
                                   i, ie2, face2, iel, ntemp);
                        }
                    }
                }
            }
            ...
            if (fassign[iel][i]) {
                ...
                // if face i is of type 2 or iel doesn't have a neighbor
                // element, assign new mortar point indices to interior mortar
                // points of face i of iel.
                cb = cbc[iel][i];
                if (cb == 1 || cb == 0) {
            ...
            if (cb == 3) {
                for (iii = 0; iii < 2; iii++) {
                    for (jjj = 0; jjj < 2; jjj++) {
                        ...
                        idmo[iel][i][jjj][iii][0][0] =
                            idmo[ntemp][jface][0][0][0][0];
                        idmo[iel][i][jjj][iii][0][LX1 - 1] =
                            idmo[ntemp][jface][1][0][0][LX1 - 1];
                        idmo[iel][i][jjj][iii][LX1 - 1][0] =
                            idmo[ntemp][jface][0][1][LX1 - 1][0];
        ...
        if (if_temp) {
            if (ifntempx[iintempx[l]] != -1) {
                ...
                while ((l != 1 && itemp <= face_l2[l]) ||
                       (l == 1 && itemp >= face_l2[l])) {
                    // lc is the local corner number of this vertex on face
                    // face_a[itemp]
                    //  on the neighbor element of iel, neighbored by a face
                    //  face_a[l]
                    lc = local_corner[face_a[itemp]][j[l]];
                    // temp is the vertex index of this vertex on the neighbor
                    // element neighbored by an edge
                    ...
                    if (ntemp == -1) {
                        ...
                        if (ntemp != -1) {
                            if (ifsame(ntemp, c_f[jjface[face_a[itemp]]][lc],
                                       nbe, j[l])) {
                                ntempx[temp] = ntemp;
                                ifntempx[temp] = ntemp;
                                // nnb[itemp] records the neighbor element
                                // neighbored by an edge only
                                nnb[itemp] = ntemp;
                            }
                        }
                ...
                if (ifntempx[iintempx[face_l1[l]]] != -1) {
                    ...
                    // ntemp records the last neighbor element neighbored by an
                    // edge with element iel
                    ntemp = sje[nbe][face_a[itemp]][jj][ii];
                    // if conforming
                    if (ntemp == -1) {
                        ntemp = sje[nbe][face_a[itemp]][0][0];
                    ...
                    // if the neighbor element neighbored by face
                    // face_a[face_l2[l]] does not exist
                }
