...
void convect(logical ifmortar)
{
    ...
    for (iel = 0; iel < nelt; iel++) {
        isize = size_e[iel];
        /*
        xloc[i] is the location of i'th collocation in x direction in an
        element. yloc[i] is the location of j'th collocation in y direction in
        an element. zloc[i] is the location of k'th collocation in z direction
        in an element.
        */
        for (i = 0; i < LX1; i++) {
        ...
        for (k = 0; k < LX1; k++) {
            for (j = 0; j < LX1; j++) {
                for (i = 0; i < LX1; i++) {
                    r2 = pow(xloc[i] - xx0[0], 2.0) +
                         pow(yloc[j] - yy0[0], 2.0) +
        ...
        for (k = 0; k < LX1; k++) {
            for (j = 0; j < LX1; j++) {
                for (i = 0; i < LX1; i++) {
                    r2 = pow(xloc[i] - xx0[1], 2.0) +
                         pow(yloc[j] - yy0[1], 2.0) +
        ...
        for (k = 0; k < LX1; k++) {
            for (j = 0; j < LX1; j++) {
                for (i = 0; i < LX1; i++) {
                    r2 = pow(xloc[i] - xx0[2], 2.0) +
                         pow(yloc[j] - yy0[2], 2.0) +
                    ...
                    tempa[k][j][i] =
                        sixth * (rk1[k][j][i] + 2.0 * rk2[k][j][i] +
                                 2.0 * rk3[k][j][i] + rk4[k][j][i]);
                }
            }
        }
        ...
        for (k = 0; k < LX1; k++) {
            for (j = 0; j < LX1; j++) {
                for (i = 0; i < LX1; i++) {
                    trhs[iel][k][j][i] =
                        bm1_s[isize][k][j][i] *
                        (ta1[iel][k][j][i] * rdtime + tempa[k][j][i]);
                    ta1[iel][k][j][i] =
                        ta1[iel][k][j][i] + tempa[k][j][i] * dtime;
                }
            }
        }
    }
    ...
    if (timeron)
        timer_stop(t_transfb_c);

    for (i = 0; i < nmor; i++) {
        tmort[i] = tmort[i] / mormult[i];
    }
    if (timeron)
        timer_stop(t_convect);
}
