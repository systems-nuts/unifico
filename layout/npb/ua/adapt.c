    ...
    *ifmortar = false;
    ...
    while (true) {
        ...
        if (if_refine) {
            ifrepeat = true;
            while (ifrepeat) {
                // Check with restriction, unmark elements that cannot be
                // refined.
                // Elements preventing desired refinement will be marked to be
                // refined.
    ...
    while (true) {
        ...
        if (if_coarsen) {
            ...
            if (if_coarsen) {
                // ifmortar=true indicates the grid changed, i.e. the mortar
                // points indices need to be regenerated on the new grid.
                *ifmortar = true;
            }
            else {
                break;
    ...
    for (miel = 0; miel < nelt; miel++) {
        ...
        if (ich[iel] == 2) {
            ...
            if (!btest(ic, 0) && !btest(ic, 1) && !btest(ic, 2)) {
                ...
                // front[miel] will be used to calculate (potentially in
                // parallel)
                //             how many elements with seuqnece numbers less than
                //             miel will be coarsened.
                // skip[]      marks that an element will no longer exist after
                // merge.
...
static void do_refine(logical *ifmortar, int *irefine)
{
    ...
    for (index = 0; index < num_refine; index++) {
        // miel is old morton index and mielnew is new morton index after
        // refinement.
        miel = action[index];
        mielnew = miel + (front[miel] - 1) * 7;
        iel = mt_to_id_old[miel];
        ...
        for (ndir = 0; ndir < 3; ndir++) {
            for (facedir = 0; facedir <= 1; facedir++) {
                ...
                // update neighbor information of the four child elements on
                // each face of the parent element
                for (k = 0; k < 4; k++) {
                    cbc[le[k]][i] = 2;
                    sje[le[k]][i][0][0] = ne[k];
                    ijel[le[k]][i][0] = 0;
        ...
        if (ich[iel] == 4) {
            ...
            for (i = 0; i < NSIDES; i++) {
                ...
                if (cbc[iel][i] == 1) {
                    ...
                    // check iel's neighbor, neighbored by an edge on face i,
                    // which must be a face neighbor of ntemp
                    for (iface = 0; iface < NSIDES; iface++) {
                        if (iface != i && iface != jface) {
                            // if edge neighbors are larger than iel, mark them
                            // to be refined
                            if (cbc[ntemp][iface] == 2) {
                                nntemp = sje[ntemp][iface][0][0];
                                // ifcor is to make sure the edge neighbor exist
                                if (ich[nntemp] != 4 &&
                                    ifcor(iel, nntemp, i, iface)) {
                                    ich[nntemp] = 4;
                                }
                            }
                        }
                    }
                    // if face neighbor are of the same size of iel, check edge
                    // neighbors
                }
                else if (cbc[iel][i] == 2) {
                    for (iface = 0; iface < NSIDES; iface++) {
    ...
    for (iz = 0; iz < LX1; iz++) {
        for (kk = 0; kk < LX1; kk++) {
            for (jj = 0; jj < LX1; jj++) {
                for (ii = 0; ii < LX1; ii++) {
                    y[jj][iz][ii] =
                        y[jj][iz][ii] + ytwo[0][iz][kk][ii] * ixtmc1[jj][kk];
