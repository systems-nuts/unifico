...
void transf(double tmor[], double tx[])
{
    ...
    for (ie = 0; ie < nelt; ie++) {
        for (iface = 0; iface < NSIDES; iface++) {
            ...
            if (nnje == 2) {
                // nonconforming faces have four pieces of mortar, first map
                // them to two intermediate mortars, stored in tmp
                r_init((double *)tmp, LX1 * LX1 * 2, 0.0);

                for (ije1 = 0; ije1 < nnje; ije1++) {
                    for (ije2 = 0; ije2 < nnje; ije2++) {
                        for (col = 0; col < LX1; col++) {
                            // in each row col, when coloumn i=1 or LX1, the
                            // value in tmor is copied to tmp
                            i = v_end[ije2];
                            ig = idmo[ie][iface][ije2][ije1][col][i];
                            tmp[ije1][col][i] = tmor[ig];

                            // in each row col, value in the interior three
                            // collocation points is computed by apply mapping
                            // matrix qbnew to tmor
                            for (i = 1; i < LX1 - 1; i++) {
                ...
                for (ije1 = 0; ije1 < nnje; ije1++) {
                    ...
                    // the value on the three interior collocation points, tx,
                    // is computed by applying mapping matrices qbnew to tmp.
                    // the mapping result is divided by 2, because there will be
                    // duplicated contribution from another face sharing this
                    // edge.
                    col = 0;
                    for (i = 1; i < LX1 - 1; i++) {
                    ...
                    for (col = 1; col < LX1 - 1; col++) {
                        // when i=0 or LX1-1, the collocation points are also on
                        // an edge of
                        //  the face, so the mapping result also needs to be
                        //  divided by 2
                        i = v_end[ije1];
                        il = idel[ie][iface][i][col];
...
void transfb(double tmor[], double tx[])
{
    ...
    for (ie = 0; ie < nelt; ie++) {
        for (iface = 0; iface < NSIDES; iface++) {
            ...
            // only 1/3 of the value is summed, since there will be two
            // duplicated contributions from the other two faces sharing this
            // vertex
            ...
            if (nnje == 2) {
                ...
                // nonconforming faces have four pieces of mortar, first map tx
                // to two intermediate mortars stored in temp
                for (ije2 = 0; ije2 < nnje; ije2++) {
                    shift = ije2;
                    for (col = 0; col < LX1; col++) {
                        // For mortar points on face edge (top and bottom), copy
                        // the value from tx to temp
                        il = idel[ie][iface][v_end[ije2]][col];
                        temp[ije2][v_end[ije2]][col] = tx[il];

                        // For mortar points on face edge (top and bottom),
                        // calculate the interior points' contribution to them,
                        // i.e. top()
                        j = v_end[ije2];
                        tmp = 0.0;
                        for (i = 1; i < LX1 - 1; i++) {
                        ...
                        // Use mapping matrices qbnew to map the value from tx
                        // to temp for mortar points not on the top bottom face
                        // edge.
                ...
                for (ije1 = 0; ije1 < nnje; ije1++) {
                    ...
                    for (ije2 = 0; ije2 < nnje; ije2++) {
                        ...
                        // for each column of collocation points on a piece of
                        // mortar
                        for (col = 2 - shift - 1; col < LX1 - shift; col++) {

                            // For the end point, which is on an edge (local
                            // edge 1,3), the contribution is halved since there
                            // will be duplicated contribution from another face
                            // sharing this edge.
                            ...
                            // In each row of collocation points on a piece of
                            // mortar, sum the contributions from interior
                            // collocation points (i=1,LX1-2)
                            for (j = 0; j < LX1; j++) {
                                tmp = 0.0;
                                for (i = 1; i < LX1 - 1; i++) {
                                    tmp = tmp + qbnew[ije2][j][i - 1] *
                                                    temp[ije1][col][i];
                                }
                                ig = idmo[ie][iface][ije2][ije1][col][j];
                                tmor[ig] = tmor[ig] + tmp;
                        ...
                        // For tmor on local edge 0 and 2, tmp is the
                        // contribution from an edge, so it is halved because of
                        // duplicated contribution from another face sharing
                        // this edge. tmp1 is contribution from face interior.
                        ...
                        for (j = 0; j < LX1; j++) {
                            ...
                            for (i = 1; i < LX1 - 1; i++) {
                                tmp = tmp + qbnew[ije2][j][i - 1] *
                                                temp[ije1][col][i];
                                tmp1 =
                                    tmp1 + qbnew[ije2][j][i - 1] * top[ije1][i];
                            }
                            ig = idmo[ie][iface][ije2][ije1][col][j];
