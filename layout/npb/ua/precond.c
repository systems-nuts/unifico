...
void setpcmo_pre()
{
    ...
    for (col = 0; col < LX1; col++) {
        ...
        for (j = 1; j < LX1 - 1; j++) {
            for (i = 0; i < LX1; i++) {
                tcpre[j][col] =
                    tcpre[j][col] + qbnew[0][i][j - 1] * tmp[i][col];
            }
        }
    }
                    ...
                    if (!edgevis[iel][iside][_enum]) {
                        ...
                        if (cbc[iel][face2] == 2) {
                            ...
                            if (cbc[nb1][iside] == 2) {
                                ...
                                // Compute the preconditioner on local edge
                                // _enum on face iside of element iel, 1 is
                                // neighborhood information got by examing
                                // neighbors(nb1). For detailed meaning of 1,
                                // see subroutine com_dpc.

                                com_dpc(iside, iel, _enum, 1, sizei);
                                nb2 = sje[nb1][iside][0][0];
                                edgevis[nb2][jjface[face2]]
                                       [op[e_face2[iside][_enum]]] = true;
                            }
                    ...
                    for (nn2 = 0; nn2 < 2; nn2++) {
                        for (j = 1; j < LX1 - 1; j++) {
                            for (i = 1; i < LX1 - 1; i++) {
                                ...
                                dpcmor[imor] =
                                    1.0 / (pcmor_nc1[sizei][nn2][nn1][j][i] +
                                           pcmor_c[sizei + 1][j][i]);
                            }
                        }
                    }
...
static void pc_corner(int imor)
{
    ...
    for (inemo = 0; inemo <= nemo[imor]; inemo++) {
        ...
        for (iiface = 0; iiface < 3; iiface++) {
            ...
            // sface sums the number of nonconforming faces sharing this vertex
            // on one element
            if (cbc[ie][iface] == 3) {
                sface = sface + 1;
            }
