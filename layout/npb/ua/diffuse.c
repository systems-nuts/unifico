...
void diffusion(logical ifmortar)
{
    ...
    for (iter = 1; iter <= nmxh; iter++) {
        if (iter > 1) {
            rho_aux = 0.0;
            // pdiffp and ppmor are combined to generate q_m+1 in the
            // specification rho_aux is (q_m+1,r_m+1)
            for (ie = 0; ie < nelt; ie++) {
                for (k = 0; k < LX1; k++) {
                    for (j = 0; j < LX1; j++) {
                        for (i = 0; i < LX1; i++) {
        ...
        if (timeron)
            timer_start(t_transfb);
        transfb(ppmor, (double *)pdiffp);
        if (timeron)
            timer_stop(t_transfb);
        ...
        for (ie = 0; ie < nelt; ie++) {
            for (k = 0; k < LX1; k++) {
                for (j = 0; j < LX1; j++) {
                    for (i = 0; i < LX1; i++) {
                        cona = cona + pdiff[ie][k][j][i] * pdiffp[ie][k][j][i] *
                                          tmult[ie][k][j][i];
                    }
                }
            }
