...
int main(int argc, char *argv[])
{
    ...
    if (argc > 3 || (argc > 1 && !isdigit(argv[1][0]))) {
        ...
        fprintf(stderr,
                "The last argument, (a parameter file) can be skipped\n");
        exit(1);
    }
...
int32 DC(ADC_VIEW_PARS *adcpp)
{
    ...
    if (!adccntlp) {
        PutErrMsg(
            "ParRun.NewAdcViewCntl: returned NULL") return ADC_INTERNAL_ERROR;
    }
    else {
    ...
    c_print_results("DC", adcpp->clss, (int)adcpp->nInputRecs, 0, 0, 1, t_total,
                    ...
                    "Tuples generated", verified, NPBVERSION, COMPILETIME, CC,
