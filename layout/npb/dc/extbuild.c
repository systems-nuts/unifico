...
int32 SharedSortAggregate(ADC_VIEW_CNTL *avp)
{
    ...
    for (iib = 1; iib <= nsgs; iib++) {
        ...
        for (ibOffset = 0, iRec = 1; iRec <= ncur; iRec++) {
            ...
            SelectToView(attrs, avp->selection, currBuf, avp->nd, avp->nm,
                         avp->nv);
                ...
                if (currV == prevV) {
                    ...
                    if (avp->tree->memoryIsFull) {
                        avp->chunksParams[avp->numberOfChunks].curChunkNum =
                            avp->tree->count;
                        avp->chunksParams[avp->numberOfChunks].chunkOffset =
                            chunkOffset;
                        (avp->numberOfChunks)++;
                        ...
                        chunkOffset +=
                            (uint64)(avp->tree->count * avp->outRecSize);
                        retCode =
                            WriteChunkToDisk(avp->outRecSize, avp->fileOfChunks,
                    ...
                    if (avp->numberOfChunks && avp->tree->count != 0) {
                        ...
                        avp->chunksParams[avp->numberOfChunks].chunkOffset =
                            chunkOffset;
                        (avp->numberOfChunks)++;
                        chunkOffset += (uint64)(avp->tree->count *
                                                (4 * avp->nv + 8 * avp->nm));
                        retCode =
                            WriteChunkToDisk(avp->outRecSize, avp->fileOfChunks,
                                             avp->tree->root.left, avp->logf);
                    ...
                    if (!avp->numberOfChunks) {
                        avp->nViewRows += avp->tree->count;
                        retCode = WriteViewToDiskCS(avp, avp->tree->root.left,
                                                    &ordern);
                        if (retCode != ADC_OK) {
                            fprintf(
                                stderr,
                        ...
                        if (retCode != ADC_OK) {
                            fprintf(
                                stderr,
                                "SharedSortAggregate.MultiWayMerge: failed.\n");
                            return retCode;
                        }
                    }
                    InitializeTree(avp->tree, avp->nv, avp->nm);
...
int32 PrefixedAggregate(ADC_VIEW_CNTL *avp, FILE *iof)
{
    ...
    for (iRec = 1; iRec <= avp->nRowsToRead; iRec++) {
        ...
        if (iRec == 1)
            ...
            compRes =
                KeyComp(&currBuf[2 * avp->nm], &prevBuf[2 * avp->nm], avp->nv);
            ...
            case -1:
                fprintf(stderr,
                        "PrefixedAggregate: wrong parent view order.\n");
                exit(1);
                break;
            default:
            ...
            if (nOut == nOutBufRecs) {
                ...
                WriteToFile(avp->memPool, nOut * avp->outRecSize, 1, iof,
                            stderr);
...
int32 RunFormation(ADC_VIEW_CNTL *avp, FILE *inpf)
{
    ...
    for (iRec = 1; iRec <= avp->nRowsToRead; iRec++) {
        ...
        if (avp->tree->memoryIsFull) {
            ...
            if (avp->numberOfChunks >= MAX_NUM_OF_CHUNKS) {
                fprintf(stderr,
                        "RunFormation: Too many chunks were created.\n");
                return ADC_INTERNAL_ERROR;
            }
                  ...
                  uint32 regSubChunkSize, CHUNKS *chunks, FILE *fileOfChunks)
{
    if (chunks[iChunk].curChunkNum > 0) {
        if (chunks[iChunk].curChunkNum < regSubChunkSize) {
            ...
            SeekAndReadNextSubChunk(
                multiChunkBuffer, iChunk * regSubChunkSize * mwBufRecSizeInInt,
                fileOfChunks, chunkRecSize, chunks[iChunk].chunkOffset,
                regSubChunkSize);
...
int32 MultiWayMerge(ADC_VIEW_CNTL *avp)
{
    ...
    while (1) {
        ...
        for (iChunk = min_r_chunk; iChunk < avp->numberOfChunks; iChunk++) {
            ...
            if (avp->chunksParams[iChunk].curSubChunk > 0) {
                iPos =
                    mwBufRecSizeInInt * (iChunk * regSubChunkSize +
                                         avp->chunksParams[iChunk].posSubChunk);
                memcpy(&r_buf[0], &multiChunkBuffer[iPos], avp->outRecSize);

                compRes = KeyComp(&r_buf[2 * avp->nm], &min_r_buf[2 * avp->nm],
                                  avp->nv);
                if (compRes < 0) {
        ...
        if (first_one) {
            ...
            if (!compRes) {
                ...
                WriteToFile(outputBuffer, avp->outRecSize, 1, avp->viewFile,
                            stderr);
                avp->nViewRows++;
                for (i = 0; i < avp->nm; i++) {
                    avp->mSums[i] += m_outputBuffer[i];
                    avp->checksums[i] +=
                        avp->nViewRows * m_outputBuffer[i] % measbound;
                }
                memcpy(&outputBuffer[0], &min_r_buf[0], avp->outRecSize);
            }
        }
...
ADC_VIEW_CNTL *NewAdcViewCntl(ADC_VIEW_PARS *adcpp, uint32 pnum)
{
    ...
    adccntl->chunksParams =
        (CHUNKS *)malloc(MAX_NUM_OF_CHUNKS * sizeof(CHUNKS));
    if (adccntl->chunksParams == NULL) {
        fprintf(adccntl->logf,
                "NewAdcViewCntl: Cannot allocate 'chunksParsms'\n");
    ...
    if (adccntl->lpp == NULL) {
        fprintf(adccntl->logf,
                "\n Not enough space to allocate %ld byte for a layer "
                "reference array.",
    ...
    while (fread(&binRepTuple, 8, 1, adccntlp->groupbyFile)) {
...
#endif /* OPTIMIZATION */
        {
            if (smp != noneParent) {
                ...
                if ((retCode = RunFormation(adccntlp, adccntlp->inpf)) !=
                    ADC_OK) {
                    fprintf(stderr,
                            "ComputrGivenGroupbys.RunFormation failed.\n");
                    return retCode;
            ...
            if (!adccntlp->numberOfChunks) {
                ...
                retCode = WriteViewToDiskCS(adccntlp, adccntlp->tree->root.left,
                                            &ordern);
                if (retCode != ADC_OK) {
                    fprintf(
                        stderr,
                ...
                if (retCode != ADC_OK) {
                    fprintf(stderr,
                            "ComputeGivenGroupbys.MultiWayMerge failed.\n");
                    return retCode;
                }
            }
        }
