 ...
 * @param num_arch_live number of arch-specific live value records copied to
 * arch_live
 ...
 */
static bool
create_call_site_metadata(bin *b, uint64_t start_id, stack_map_section *sm,
                          size_t num_sm, size_t *num_sites, call_site **cs,
...
{
    size_t num_shdr, i, added = 0, cur_offset, num_sites, num_live,
                        num_arch_live, num_unwind;
    ...
    if (!(scn = get_section_by_name(b->e, unwind_sec)))
        return FIND_SECTION_FAILED;
    if (!(shdr = elf64_getshdr(scn)))
        return READ_ELF_FAILED;
    if (!shdr->sh_size || !shdr->sh_entsize)
    ...
    if (!(unwind = get_section_data(scn)))
        return READ_ELF_FAILED;
    if (!create_call_site_metadata(
            b, start_id, sm, num_sm, &num_sites, &id_sites, &num_live,
            &live_vals, &num_arch_live, &archlive, num_unwind, unwind))
    ...
    if ((scn = get_section_by_name(b->e, sec_name)))
        ...
        ret =
            add_section(b->e, sec_name, num_sites, sizeof(call_site), id_sites);
    if (ret)
        return ret;
    added++;
    ...
    if ((scn = get_section_by_name(b->e, sec_name)))
        ret =
            update_section(b->e, scn, num_sites, sizeof(call_site), addr_sites);
    else
        ret = add_section(b->e, sec_name, num_sites, sizeof(call_site),
                          addr_sites);
    if (ret)
        return ret;
    added++;
    ...
    if ((scn = get_section_by_name(b->e, sec_name)))
        ret =
            update_section(b->e, scn, num_live, sizeof(live_value), live_vals);
    else
        ret = add_section(b->e, sec_name, num_live, sizeof(live_value),
                          live_vals);
    if (ret)
        return ret;
    added++;
    ...
    if ((scn = get_section_by_name(b->e, sec_name)))
        ret = update_section(b->e, scn, num_arch_live, sizeof(arch_live_value),
                             archlive);
    else
        ret = add_section(b->e, sec_name, num_arch_live,
                          sizeof(arch_live_value), archlive);
    if (ret)
        return ret;
    added++;

    /* Calculate offset of last non-stack-transform section */
    if (elf_getshdrnum(b->e, &num_shdr) == -1)
        return READ_ELF_FAILED;
    if (!(scn = elf_getscn(b->e, num_shdr - (added + 1))))
        return READ_ELF_FAILED;
    if (!(shdr = elf64_getshdr(scn)))
        return READ_ELF_FAILED;
                          ...
                          size_t num_addrs, const unwind_addr *addrs)
{
    ...
    if (verbose)
        printf(
            "Creating metadata for %lu call sites, %lu location records & %lu "
...
static int sort_unwind_addr(const void *a, const void *b)
{
    ...
    if (ua_a->addr < ua_b->addr)
        return -1;
    else if (ua_a->addr == ua_b->addr)
        return 0;
    else
        return 1;
}
...
static int sort_addr(const void *a, const void *b)
{
    ...
    if (cs_a->addr < cs_b->addr)
        return -1;
    else if (cs_a->addr == cs_b->addr)
        return 0;
    else
        return 1;
}
