...
bool check_elf_ehdr(Elf *e)
{
    ...
    if (elf_kind(e) != ELF_K_ELF)
        return false; // is it an ELF object?
    if (gelf_getclass(e) != ELFCLASS64)
        return false; // is it 64-bit?

    /* Identity bytes which must be checked manually */
    if (!(id = elf_getident(e, NULL)))
        return false;
    if (id[EI_DATA] != ELFDATA2LSB)
        return false; // is it 2's complement, little-endian?
    // TODO clang produces SysV binaries for AArch64 but GNU for x86-64
    // if(id[EI_OSABI] != ELFOSABI_SYSV) return false; // is it UNIX - System V
    // ABI?

    /* Architecture checks */
    if (!(ehdr = elf64_getehdr(e)))
        return false;
    if (ehdr->e_machine != EM_AARCH64 && ehdr->e_machine != EM_PPC64 &&
...
Elf_Scn *get_section_by_name(Elf *e, const char *name)
{
    ...
    if (elf_getshdrstrndx(e, &shdrstrndx))
        return NULL;
...
Elf_Scn *get_sym_tab(Elf *e)
{
    ...
    while ((scn = elf_nextscn(e, scn))) {
        ...
        if (shdr.sh_type == SHT_SYMTAB)
            break; // According to ELF standard only
...
GElf_Sym get_sym_by_name(Elf *e, const char *name)
{
    ...
    if (!(symtab = get_sym_tab(e)))
        return ret;
    if (gelf_getshdr(symtab, &shdr) != &shdr)
        return ret;
    if (!(data = elf_getdata(symtab, data)))
        return ret;
    ...
    for (i = 0; i < num_syms; i++) {
        if (gelf_getsym(data, i, &sym) != &sym)
            return ret;
        if (!(sym_name = elf_strptr(e, shdr.sh_link, sym.st_name)))
            continue;
...
uint64_t add_section_name(Elf *e, const char *name)
{
    ...
    if (elf_getshdrstrndx(e, &shdrstrndx) != 0)
        return 0;
    if (!(scn = elf_getscn(e, shdrstrndx)))
        return 0;
                                        ...
                                        const unwind_addr *addrs)
{
    ...
    while (max >= min) {
        ...
        if (IN_RANGE(mid, addr))
            return &addrs[mid];
        else if (addr > addrs[mid].addr)
            min = mid + 1;
        else
            max = mid - 1;
    }
                  ...
                  size_t entry_size, void *buf)
{
    ...
    if (verbose)
        printf("Adding section '%s': %lu entries, %lu bytes\n", name,
               num_entries, num_entries * entry_size);

    if ((name_off = add_section_name(e, name)) == 0)
        return WRITE_ELF_FAILED;

    if (!(scn = elf_newscn(e)))
        return ADD_SECTION_FAILED;
    if (!(shdr = elf64_getshdr(scn)))
        return ADD_SECTION_FAILED;
