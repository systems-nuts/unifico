...
ret_t init_stackmap(bin *b, stack_map_section **sm_ptr, size_t *num_sm)
{
    ...
    *num_sm = 0;
    ...
    while (offset < shdr.sh_size) {
        ...
        offset +=
            stackmap_records_size(data->d_buf + offset, tmp_sm.num_records);
    }

    // Note: we should expect LLVM to generate a stackmap section for each
    // instrumented module
    if (verbose)
        printf("Found %lu stackmap section(s)\n", *num_sm);
    ...
    for (i = 0; i < *num_sm; i++) {
        ...
        if (verbose)
            printf("  Stackmap v%d, %u function(s), %u constant(s), %u "
                   "record(s)\n",
        ...
        if (verbose)
            for (j = 0; j < sm[i].num_functions; j++)
                printf("    Function %lu: %p, stack frame size = %lu byte(s), "
                       "%u unwinding records\n",
                       j, (void *)sm[i].function_records[j].func_addr,
        ...
        if (verbose)
            for (j = 0; j < sm[i].num_records; j++)
                printf("    Stack map %lu: %lu "
                       ...
                       j, sm[i].call_sites[j].id, sm[i].call_sites[j].func_idx,
