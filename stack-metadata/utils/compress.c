...
ret_t compress_bss(bin *b)
{
    ...
    if (!data->d_size) {
        if (verbose)
            printf("'.bss' already has zero file size\n");
        return SUCCESS;
    }

    if (verbose) {
        if (elf_getshdrstrndx(b->e, &shdrstrndx))
            return READ_ELF_FAILED;
    ...
    while ((scn = elf_nextscn(b->e, scn))) {
        ...
        if (verbose)
            printf("Updating section '%s' to offset 0x%lx\n",
    ...
    for (i = 0; i < nphdr; i++) {
        if (gelf_getphdr(b->e, i, &phdr) != &phdr)
            return READ_ELF_FAILED;
        /*
         * Look for the segment containing .bss.  Once we've found it, reduce
         * its file size to zero (keeping memory size the same) and update the
         * file offsets for the remaining segments.
         */
        if (phdr.p_offset == orig_offset && phdr.p_filesz == orig_size) {
            ...
            phdr.p_filesz = 0;
            if (!gelf_update_phdr(b->e, i, &phdr))
             ...
             *
             *   "Virtual addresses and file offsets for the SYSTEM V
             * architecture segments are congruent modulo 4 KB (0x1000) or
             * larger powers of 2"
             *
             * The last 3 digits (hex) of the offset & virtual address must
             * match.
             */
