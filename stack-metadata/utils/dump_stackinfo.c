...
bool dump_callsite_section(Elf_Scn *scn)
{
    ...
    for (i = 0; i < num_sites; i++)
        printf("%lu: 0x%lx, %u, %u unwind entries (offset=%lu), "
               ...
               sites[i].num_unwind, sites[i].unwind_offset, sites[i].num_live,
               sites[i].live_offset, sites[i].num_arch_live,
               sites[i].arch_live_offset);
    printf("\n");
...
bool dump_archlive_section(Elf_Scn *scn)
{
    ...
    for (i = 0; i < num_live; i++) {
        printf("%lu: value is ", i);
        if (!print_arch_val_record(&live[i]))
            return false;
    }
    printf("\n");
