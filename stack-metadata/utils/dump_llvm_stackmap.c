...
static const char *args = "hf:n:s:c:";
...
\t-c callsite : id of the callsite to examine\n\n\
...
static void parse_args(int argc, char **argv)
{
    ...
    while ((arg = getopt(argc, argv, args)) != -1) {
        ...
        case 'c': {
            char **endptr = NULL;
            callsite = strtol(optarg, endptr, 10);
            break;
        }
        case 's':
...
static inline bool dump_location(live_value *record)
{
    ...
    case SM_REGISTER:
        ...
        break;
    case SM_DIRECT:
        printf("at register %u + %d", record->regnum,
               record->offset_or_constant);
        break;
    case SM_INDIRECT:
        printf("at pointer generated by register %u + %d", record->regnum,
               record->offset_or_constant);
        break;
    case SM_CONSTANT:
        ...
        break;
    case SM_CONST_IDX:
        printf("in constant pool at index %d", record->offset_or_constant);
        break;
    default:
...
static inline bool dump_stackmaps(bin *b, stack_map_section *sm, size_t num_sm)
{
    size_t i;
    uint32_t j;
    char printed_stackmaps = false;
    int64_t func_idx; // If func_name is given, then this will be its index in
    ...
    for (i = 0; i < num_sm; i++) {
        ...
        for (j = 0; j < sm[i].num_functions; j++) {
            ...
            if (func_name && strcmp(sym_name, func_name) != 0) {
                ...
                if (!printed_stackmaps) {
                    printf("Stackmap v%u: %u functions, %u constants, %u call "
                           "sites\n",
                           sm[i].version, sm[i].num_functions,
                           sm[i].num_constants, sm[i].num_records);
                    printed_stackmaps = true;
                }
                func_idx = j;
            }

            printf("  Function %s: address=%lx, stack size=%lu, number of "
                   "unwinding "
