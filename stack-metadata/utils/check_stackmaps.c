...
static const char *help = "check-stackmaps - check LLVM stackmap sections for "
                          "matching metadata across "
                          "binaries\n\n\
\
 ...
 * So, we'll iterate over everything and print out where we find
 * inconsistencies. If `func_name` is given, we only check the callsites of this
 * function.
 */
ret_t check_stackmaps(bin *a, stack_map_section *sm_a, size_t num_sm_a, bin *b,
                      stack_map_section *sm_b, size_t num_sm_b)
{
    ret_t ret = SUCCESS;
    char buf[BUF_SIZE];
     ...
     * stackmap-insertion pass run over its IR, i.e., there's no stackmap
     * section for one of the object files included in the binary.
     */
    if (num_sm_a != num_sm_b) {
        snprintf(buf, BUF_SIZE,
                 "number of stackmaps doesn't match (%lu vs. %lu)", num_sm_a,
                 num_sm_b);
        warn(buf);
    ...
    for (i = 0; i < num_sm; i++) {
        /*
         * Errors here indicate different numbers of stackmap intrinsics
        ...
        for (j = 0; j < num_records; j++) {
            ...
            if (func_a != func_b) {
                snprintf(buf, BUF_SIZE,
                         ...
                         "%s/%lx vs. %s/%lx",
                         sm_a[i].call_sites[j].id, sym_a_name, func_a,
                         sym_b_name, func_b);
                warn(buf);
                ret = INVALID_METADATA;
            }
                ...
                /*
                 * If `func_name` is given as an argument, only check the
                 * function record with that name.
                 */
                if (func_name && strcmp(sym_a_name, func_name) != 0) {
                    continue;
                }

                qsort(sm_a[i].call_sites, num_records, sizeof(call_site_record),
                      cmp_callsites);
                qsort(sm_b[i].call_sites, num_records, sizeof(call_site_record),
                      cmp_callsites);

                /*
                 * The raw location record count may be different, so count
                 * non-duplicated records, i.e., ignore backing stack slot
                 * locations.
                 * TODO
                 */
                ...
                /*
                 * Errors here indicate different numbers of live values at the
                 * stackmap intrinsic call site.
                 */
                if (num_a != num_b) {
                    snprintf(buf, BUF_SIZE,
                             "%s: stackmap %lu has different numbers of "
                             "location records "
                             "(%u vs. %u)",
                ...
                /*
                 * Iterate over all location records (i.e., all live values) at
                 * a call site.  Errors point to different live values/different
                 * orderings of live values at the site.
                 */
                for (k = 0, l = 0; k < num_a && l < num_b; k++, l++) {
                    ...
                    if (flag_a != flag_b) {
                        snprintf(
                            buf, BUF_SIZE,
                            "%s: callsite %lu, value locations %lu/%lu have "
                    ...
                    if (flag_a != flag_b) {
                        snprintf(buf, BUF_SIZE,
                                 "%s: callsite %lu, locations %lu/%lu have "
                    ...
                    flag_a =
                        sm_a[i].call_sites[j].locations[k].offset_or_constant;
                    flag_b =
                        sm_b[i].call_sites[j].locations[l].offset_or_constant;
                    if (flag_a != flag_b) {
                        snprintf(
                            buf, BUF_SIZE,
                            "%s: callsite %lu, value locations %lu/%lu have "
                            "different location offset or  different constant "
                            "(%d vs. %d)",
                            sym_a_name, sm_a[i].call_sites[j].id, k, l, flag_a,
                            flag_b);
                        warn(buf);
                    ...
                    while (sm_a[i].call_sites[j].locations[k + 1].is_duplicate)
                        k++;
                    while (sm_b[i].call_sites[j].locations[l + 1].is_duplicate)
                        l++;
                }
                ...
                if (num_a != num_b) {
                    snprintf(buf, BUF_SIZE,
                             "%s: callsite %lu has "
                             "different number of architecture specific live "
                             "locations (%u vs %u)",
                             sym_b_name, sm_b[i].call_sites[j].id, num_a,
                             num_b);
                    warn(buf);
                    ret = DIFFERENT_STACK_LAYOUT;
                }
                ...
                for (k = 0, l = 0; k < num_a && l < num_b; k++, l++) {
                    ...
                    if (flag_a != flag_b) {
                        snprintf(buf, BUF_SIZE,
                                 "%s, callsite %lu: arch-specific live value "
                                 "%lu/%lu has "
                                 "different location type (%d vs. %d)",
                                 sym_a_name, sm_a[i].call_sites[j].id, k, l,
                                 flag_a, flag_b);
                    ...
                    int64_t value_a = sm_a[i]
                                          .call_sites[j]
                                          .arch_live[k]
                                          .operand_offset_or_constant;
                    int64_t value_b = sm_b[i]
                                          .call_sites[j]
                                          .arch_live[l]
                                          .operand_offset_or_constant;
                    if (value_a != value_b) {
                        snprintf(buf, BUF_SIZE,
                                 "%s, callsite %lu: arch-specific live value "
                                 "%lu/%lu has "
                                 "different location operand offset or "
                                 "constant (%ld vs. %ld)",
                                 sym_a_name, sm_a[i].call_sites[j].id, k, l,
                                 value_a, value_b);
                        warn(buf);
...
int main(int argc, char **argv)
{
    ...
    stack_map_section *sm_aarch64 = NULL, *sm_powerpc64 = NULL,
                      *sm_x86_64 = NULL;
    size_t num_sm_aarch64 = 0, num_sm_powerpc64 = 0, num_sm_x86_64 = 0, i, j,
           num;
    char buf[1024];
    ...
    for (i = 0; i < num; i++) {
        for (j = i + 1; j < num; j++) {
            if (binaries[i] && binaries[j]) {
                ret = check_stackmaps(binaries[i], stackmaps[i],
                                      num_stackmaps[i], binaries[j],
                                      stackmaps[j], num_stackmaps[j]);
                if (ret != SUCCESS) {
                    snprintf(buf, sizeof(buf),
                             "stackmaps in '%s' & '%s' differ",
