...
CMD1="python3.7 -m spec2017.run_spec --config-list clang_base.cfg --threads=1 \
      --bench=600,602,605,625 --iterations=3 --noreportable --tune=base -i test,train,refspeed"
CMD2="python3.7 -m spec2017.run_spec --config-list clang_base.cfg \
      --threads=1,2,4,6,8 --bench 657 --iterations=3 --noreportable --tune=base -i test,train,refspeed"
CMD3="python3.7 -m spec2017.run_spec --config-list clang_base.cfg --full-thread-run \
...
CONFIG_FILE="$SPEC_SCRIPT_DIR"/config/clang_base.cfg
...
do
  ...
  git checkout "$short_hash" -- "$CONFIG_FILE" "$EXPERIMENT_INFO"
...
git checkout $WORKING_BRANCH -- "$CONFIG_FILE" "$EXPERIMENT_INFO"
